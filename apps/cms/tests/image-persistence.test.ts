...
describe("image field persistence", () => {
  it("saves and reloads single-image metadata and alt text", async () => {
    const original = await mockCmsBackend.data.getRecord("posts", "pos-0001");
    const image = {
      src: "/mock-storage/cms-assets/posts/campaign-cover.png",
    ...
    const reloaded = await mockCmsBackend.data.getRecord("posts", saved.id);
  ...
  it("preserves gallery add, remove, reorder, and metadata across reload", async () => {
    const original = await mockCmsBackend.data.getRecord("launch-pages", "lau-0001");
    const initial = [
    ...
    const saved = await mockCmsBackend.data.saveRecord(
      "launch-pages",
      { ...original, values: { ...original.values, imageGallery: serializeImageGallery(reordered) } },
      { expectedModifiedAt: original.modifiedAt }
    );
    const reloaded = await mockCmsBackend.data.getRecord("launch-pages", saved.id);

    assert.deepEqual(parseImageGallery(reloaded.values.imageGallery), reordered);
    assert.deepEqual(parseImageGallery(reloaded.values.imageGallery).map((item) => item.src), ["/mock-storage/c.jpg", "/mock-storage/a.jpg"]);
  });
});
